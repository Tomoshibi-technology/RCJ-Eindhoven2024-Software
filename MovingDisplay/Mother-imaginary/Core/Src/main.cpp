...
int16_t now_speed_pol[2] = {0};//{|r'|, degree}、現在の極速度ベクトル、デモ用に追加
int16_t cur_speed_pol[2] = {0};//{|r'|, degree}、目標極速度ベクトル、デモ用に追加
...
//	for(int i=0; i<2; i++){cur_position_rec[i] = cur_movement[i] - position[i];}//ローカルの目標座標(直交座標)

////convert movement distance to polar coordinates
//	cur_position_pol[0] = pow(pow(cur_position_rec[0], 2) + pow(cur_position_rec[1], 2), 0.5);//ローカルの目標座標(極座標)
...
//
//	if((position[2] < cur_movement[2]) && (cur_position_pol[0] > dclr_thr)){speed ++;}//現在速度が低く、減速距離に達していないと加速
//	else if(cur_position_pol[0] <= dclr_thr && cur_position_pol[0] > zero_thr){speed -= 2;}//減速距離以下、停止距離以上だと強めに減速
//	else if(position[2] >= cur_movement[2]){speed --;}//現在速度が高いと減速
//	else{speed = 0;}//停止距離以下だと停止
//
//	if(cur_position_pol[0] <= dclr_thr && cur_position_pol[0] > zero_thr && speed < 100){speed = 100;}//減速距離で止まらない最低速度
//
//	if(speed > cur_movement[2]*3){speed = cur_movement[2]*3;}//オーバーシュート、アンダーシュート防止
//	else if(speed < 0){speed = 0;}
//
//	speed = ((speed*8) + (p_speed*2) + 10-1)/ 10;//ローパス
//	p_speed = speed;
//
//	degree = cur_position_pol[1];//目標角度へ


//現在の極速度ベクトルをもとめる
	//スピードはもう求まってるので、角度を求める
//performanceクラスで計算した目標極速度ベクトルと比較して速度制御
	//スピード、角度両方
//	now_speed_pol[0] = position[2];
//	now_speed_pol[1] = ;

//	cur_speed_pol[0] = perform[2];//くらすのなかでやります
...
//	uint8_t display_send_array[31];
	...
	if(performance.get_fixing() == 1){//固定表現する場合
		int16_t circle_fixing[8];
		int16_t fish_fixing[8];
		circle_fixing[0] = circle_position[0] - position[0];//円の相対距離算出
		...
		display_send_array[2] = circle_position[1] / 10 + 100;//上下方向
		display_send_array[3] = circle_position[2] / 10;//送信用配列に格納
